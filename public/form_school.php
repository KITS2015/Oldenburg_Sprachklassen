...
/** i18n helper: {var} Platzhalter in Strings ersetzen */
...
$optionsHtml = '<option value="">' . h(t('school.select_choose')) . '</option>';
...
$optionsHtml .= '<option value="other"' . ($curCode==='other'?' selected':'') . '>' . h(t('school.option_other')) . '</option>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($req as $f) {
        if (empty($_POST[$f])) $errors[$f] = t('val.required');
    }
    ...
    if ($isOther) {
        if ($schuleFree === '') $errors['schule_freitext'] = t('val.school_free_required');
    } else {
        if (!array_key_exists($schuleCode, $SCHULEN)) $errors['schule_aktuell'] = t('val.school_invalid');
    }
    ...
    if (($seit_monat === '' || $seit_jahr === '') && $seit_text === '') {
        $errors['seit_monat'] = t('val.since_required');
        $errors['seit_jahr']  = t('val.since_required');
        $errors['seit_text']  = t('val.since_required');
    } else {
        if ($seit_monat !== '' && !preg_match('/^(0[1-9]|1[0-2])$/', $seit_monat)) $errors['seit_monat'] = t('val.month_invalid');
        if ($seit_jahr  !== '' && (!preg_match('/^\d{4}$/', $seit_jahr) || (int)$seit_jahr < 1900 || (int)$seit_jahr > 2100)) $errors['seit_jahr'] = t('val.year_invalid');
    }

    // Jahre in Deutschland
    if (!isset($errors['jahre_in_de']) && ($_POST['jahre_in_de'] ?? '') !== '') {
        if (!preg_match('/^\d{1,2}$/', (string)$_POST['jahre_in_de'])) $errors['jahre_in_de'] = t('val.number_required');
    }

    // Herkunftsschule + Folgefeld
    $herkunft = (string)($_POST['schule_herkunft'] ?? '');
    if (!in_array($herkunft, ['ja','nein'], true)) $errors['schule_herkunft'] = t('val.choose');

    if ($herkunft === 'ja') {
        if (empty($_POST['jahre_schule_herkunft']) || !preg_match('/^\d{1,2}$/', (string)$_POST['jahre_schule_herkunft'])) {
            $errors['jahre_schule_herkunft'] = t('val.herkunft_years');
        }
    }

    // Deutsch-Niveau (optional)
    $niveau = (string)($_POST['deutsch_niveau'] ?? '');
    if ($niveau !== '' && !in_array($niveau, $GERMAN_LEVELS, true)) $errors['deutsch_niveau'] = t('val.level_invalid');
    ...
    if (count($chosen_valid) < 1) $errors['interessen'] = t('val.interests_min1');
    if (count($chosen_valid) > 2) $errors['interessen'] = t('val.interests_max2');
...
$bbsUrl  = 'https://bbs-ol.de/';
$bbsLink = '<a href="' . h($bbsUrl) . '" target="_blank" rel="noopener">' . h(parse_url($bbsUrl, PHP_URL_HOST) ?? 'bbs-ol.de') . '</a>';
?>
<div class="container py-4">

  <div class="alert alert-secondary">
    <strong><?= h(t('school.top_hint_title')) ?></strong>
    <?= t('school.top_hint_body') ?> <!-- enthält <u> -->
    <a href="<?= h($bbsUrl) ?>" target="_blank" rel="noopener"><?= h(t('school.bbs_link_label')) ?></a>.
  </div>
  ...
  <div class="card shadow border-0 rounded-4">
    <div class="card-body p-4 p-md-5">
      ...
      <?php if ($errors): ?><div class="alert alert-danger"><?= h(t('school.form_error_hint')) ?></div><?php endif; ?>
      ...
      <form method="post" action="" novalidate id="schoolForm">
        ...
        <div class="row g-4">
          ...
          <div class="col-md-6">
            <div class="vstack gap-3">
              ...
              <div>
                <label class="form-label"><?= h(t('school.label.schule_aktuell')) ?></label>
                <input type="text" id="schuleSearch" class="form-control mb-2" placeholder="<?= h(t('school.search_placeholder')) ?>" autocomplete="off">
                ...
                <div id="schuleOtherWrap" class="mt-2" style="display: <?= $curCode==='other'?'block':'none' ?>;">
                  <input type="text" name="schule_freitext" class="form-control<?= has_err('schule_freitext',$errors) ?>" placeholder="<?= h(t('school.other_placeholder')) ?>" value="<?= h($curFree) ?>">
                  <?= field_error('schule_freitext', $errors) ?>
                </div>
              </div>

              <div>
                <label class="form-label"><?= h(t('school.label.teacher')) ?></label>
              ...
              <div>
                <label class="form-label"><?= h(t('school.label.teacher_mail')) ?></label>
              ...
              <div>
                <label class="form-label d-block"><?= h(t('school.label.herkunft')) ?></label>
                ...
                <div class="<?= $herkErr ? 'border border-danger rounded p-2' : 'is-required-group' ?>">
                  <div class="btn-group" role="group" aria-label="Herkunftsland-Schule">
                    ...
                    <label class="btn btn-outline-primary" for="s_j"><?= h(t('school.yes')) ?></label>
                    <input type="radio" class="btn-check" name="schule_herkunft" id="s_n" value="nein" <?= $herk==='nein'?'checked':''; ?>>
                    <label class="btn btn-outline-primary" for="s_n"><?= h(t('school.no')) ?></label>
                  </div>
                </div>
                <?= field_error('schule_herkunft', $errors) ?>

                <div id="jahre_herkunft_wrap" class="mt-3" style="display:none;">
                  <label class="form-label"><?= h(t('school.label.herkunft_years')) ?></label>
          ...
          <div class="col-md-6">
            <div class="vstack gap-3">
              ...
              <div>
                <label class="form-label"><?= h(t('school.label.since')) ?></label>
                <div class="row g-2">
                  <div class="col-5">
                    <select name="seit_monat" class="form-select is-required<?= has_err('seit_monat',$errors) ?>">
                      <option value=""><?= h(t('school.since_month')) ?></option>
                      <?php for ($m=1; $m<=12; $m++): $mm = str_pad((string)$m, 2, '0', STR_PAD_LEFT); ?>
                        <option value="<?= h($mm) ?>" <?= $seit_monat_alt===$mm ? 'selected' : '' ?>><?= h($mm) ?></option>
                      <?php endfor; ?>
                    </select>
                  </div>
                  <div class="col-7">
                    <input name="seit_jahr" class="form-control is-required<?= has_err('seit_jahr',$errors) ?>" inputmode="numeric" pattern="\d{4}" placeholder="<?= h(t('school.since_year_ph')) ?>" value="<?= h((string)$seit_jahr_alt) ?>">
                  </div>
                </div>
                <div class="form-text"><?= t('school.since_help') /* enthält <strong> */ ?></div>
                <?= field_error('seit_monat', $errors) ?>
                <?= field_error('seit_jahr', $errors) ?>
              </div>

              <div>
                <label class="form-label"><?= t('school.label.since_text') /* enthält „…“ */ ?></label>
                <input name="seit_text" class="form-control is-required<?= has_err('seit_text',$errors) ?>" value="<?= h((string)$seit_text_alt) ?>">
                <?= field_error('seit_text', $errors) ?>
              </div>

              <div>
                <label class="form-label"><?= h(t('school.label.years_in_de')) ?></label>
                ...
                <div class="form-text"><?= tr('school.years_in_de_help', ['link' => $bbsLink]) /* enthält HTML-Link */ ?></div>
              </div>

              <div>
                <label class="form-label"><?= h(t('school.label.family_lang')) ?></label>
          ...
          <!-- Interessen -->
          <div class="col-12">
            <label class="form-label mt-2"><?= h(t('school.label.interests')) ?></label>
        ...
        <div class="mt-4 d-flex gap-2">
          <a href="<?= h(url_with_lang('/form_personal.php')) ?>" class="btn btn-outline-secondary"><?= h(t('school.btn.back')) ?></a>
          <button class="btn btn-primary"><?= h(t('school.btn.next')) ?></button>
        </div>
      </form>
    </div>
  </div>
...
(function(){
  ...
  var linkHtml = <?= json_encode($bbsLink, JSON_UNESCAPED_UNICODE) ?>;

  var msgYears = <?= json_encode(t('js.hint_years_gt3'), JSON_UNESCAPED_UNICODE) ?>;
  var msgNiv   = <?= json_encode(t('js.hint_level_b1p'), JSON_UNESCAPED_UNICODE) ?>;
  ...
  function evaluate(){
    var years = parseInt((jahre && jahre.value ? jahre.value : '').trim(), 10);
    var niv   = (niveau && niveau.value ? niveau.value : '').toUpperCase();
    var msgs = [];

    if (!isNaN(years) && years > 3) msgs.push(fmt(msgYears, {link: linkHtml}));
    if (['B1','B2','C1','C2'].indexOf(niv) !== -1) msgs.push(fmt(msgNiv, {link: linkHtml}));

    if (msgs.length) {
      hints.innerHTML = msgs.map(function(m){ return '<div>' + m + '</div>'; }).join('');
