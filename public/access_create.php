...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string)($_POST['action'] ?? '') === 'cancel') {
    ...
    // Sprache möglichst behalten (falls vorhanden)
    $redirLang = strtolower((string)($_GET['lang'] ?? $_POST['lang'] ?? $_COOKIE['lang'] ?? ''));
    $qs = $redirLang !== '' ? ('?lang=' . rawurlencode($redirLang)) : '';
    header('Location: /access_create.php' . $qs);
    exit;
}

// ------------------------------------------------------------
// Sprache: aus URL (?lang=xx) bevorzugt, sonst Cookie, sonst 'de'
// Zusätzlich: Cookie setzen/auffrischen, damit Tabs & Navigation stabil bleiben
$languages = [
    'de' => 'Deutsch','en' => 'English','fr' => 'Français','uk' => 'Українська',
    'ar' => 'العربية','ru' => 'Русский','tr' => 'Türkçe','fa' => 'فارسی',
];

$lang = strtolower((string)($_GET['lang'] ?? $_POST['lang'] ?? $_COOKIE['lang'] ?? 'de'));
if (!array_key_exists($lang, $languages)) { $lang = 'de'; }

$rtl = in_array($lang, ['ar','fa'], true);
$dir = $rtl ? 'rtl' : 'ltr';

// Cookie setzen (12 Monate), damit Sprache später ohne Query stabil ist
...
// Generische Redirect-Helper: behält lang bei
function redirect_with_lang(string $path, string $lang, array $extra = []): void {
    $qs = array_merge($extra, ['lang' => $lang]);
...
if ($step === 'do_login') {
    ...
    if (!$errors) {
        ...
        if (!$ok) {
            ...
            // Zielseite: Portal
            redirect_with_lang('/access_portal.php', $lang);
        }
    }
    ...
    if (!$errors) {
        ...
        if ($acc && (int)($acc['email_verified'] ?? 0) === 1) {
            ...
            $_SESSION['email_verify'] = [
                ...
                'exp'   => time() + 600,
                'tries' => 0,
                'lang'  => $lang, // Sprache merken (optional)
            ];
...
// URLs mit lang beibehalten
$url_login    = '/access_create.php?' . http_build_query(['mode' => 'login', 'lang' => $lang]);
$url_register = '/access_create.php?' . http_build_query(['mode' => 'register', 'lang' => $lang]);
?>
<style>
      ...
      <?php if ($mode === 'login'): ?>
        ...
        <form method="post" class="vstack gap-3">
          ...
          <div class="d-flex gap-2 mt-2">
            ...
            <a href="/index.php?<?= h(http_build_query(['lang' => $lang])) ?>" class="btn btn-outline-secondary">
              <?= h(t('access_create.back')) ?>
            </a>
