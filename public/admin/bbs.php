...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_verify($csrf)) {
        ...
        if ($action === 'create') {
            ...
            $kurz    = trim((string)($_POST['bbs_kurz'] ?? ''));

            if ($kurz !== '' && mb_strlen($kurz) > 10) {
                $flash = 'Kurzbezeichnung max. 10 Zeichen.';
                $flashType = 'danger';
            } elseif ($schulnr === '' || $bez === '') {
                ...
                try {
                    $st = $pdo->prepare("
                        INSERT INTO bbs (bbs_schulnummer, bbs_bezeichnung, bbs_kurz, rest_token_hash, is_active)
                        VALUES (?, ?, ?, ?, 1)
                    ");
                    $st->execute([$schulnr, $bez, ($kurz !== '' ? $kurz : null), $hash]);
        ...
        if ($action === 'update_kurz') {
            ...
            if ($bbsId <= 0) {
                ...
                $flashType = 'danger';
            } elseif ($kurz !== '' && mb_strlen($kurz) > 10) {
                $flash = 'Kurzbezeichnung max. 10 Zeichen.';
                $flashType = 'danger';
            } else {
                $st = $pdo->prepare("UPDATE bbs SET bbs_kurz=?, updated_at=NOW() WHERE bbs_id=?");
                $st->execute([($kurz !== '' ? $kurz : null), $bbsId]);
                header('Location: /admin/bbs.php');
                exit;
            }
        }
...
// Liste laden
$rows = $pdo->query("
    SELECT bbs_id, bbs_schulnummer, bbs_bezeichnung, bbs_kurz, is_active, created_at, updated_at,
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h1 class="h4 mb-1">BBS / API-Clients</h1>
    ...
    <div class="card-body">
        ...
        <form method="post" action="/admin/bbs.php" class="row g-2">
            ...
            <div class="col-12 col-md-2">
                <label class="form-label">Kurz (max 10)</label>
                <input class="form-control" name="bbs_kurz" maxlength="10" placeholder="z.B. bbs3">
            </div>
...
<div class="card shadow-sm">
    <div class="table-responsive">
        <table class="table table-sm table-hover align-middle mb-0">
            ...
            <tbody>
            <?php if (!$rows): ?>
                <tr><td colspan="7" class="text-muted p-3">Noch keine BBS angelegt.</td></tr>
            <?php else: ?>
                <?php foreach ($rows as $r): ?>
                    ...
                    <tr>
                        ...
                        <td style="min-width:140px;">
                            <form method="post" action="/admin/bbs.php" class="d-flex gap-2">
                                <input type="hidden" name="csrf_token" value="<?php echo h(csrf_token()); ?>">
                                <input type="hidden" name="action" value="update_kurz">
                                <input type="hidden" name="bbs_id" value="<?php echo (int)$r['bbs_id']; ?>">
                                <input class="form-control form-control-sm" name="bbs_kurz" maxlength="10"
                                       value="<?php echo h((string)($r['bbs_kurz'] ?? '')); ?>" placeholder="(leer)">
                        ...
                        <td class="text-end">
                            <form method="post" action="/admin/bbs.php" class="d-inline">
                                ...
                                <input type="hidden" name="bbs_id" value="<?php echo (int)$r['bbs_id']; ?>">
                                <button class="btn btn-outline-primary btn-sm" type="submit">Token rotieren</button>
                            </form>
                            ...
                            <?php if ($active): ?>
                                <form method="post" action="/admin/bbs.php" class="d-inline">
                                    ...
                                    <button class="btn btn-outline-secondary btn-sm" type="submit">Deaktivieren</button>
                                </form>
                            <?php else: ?>
                                <form method="post" action="/admin/bbs.php" class="d-inline">
                                    <input type="hidden" name="csrf_token" value="<?php echo h(csrf_token()); ?>">
                                    ...
                                    <button class="btn btn-outline-success btn-sm" type="submit">Aktivieren</button>
                                </form>
                            <?php endif; ?>
                        </td>
                    </tr>
